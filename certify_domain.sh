...
# Dominio a certificar (reemplaza con tu dominio real)
...
cat > "/etc/nginx/sites-available/evolution_api" <<EOF
server {
    listen 80;
    server_name $DOMAIN;
...
ln -sf /etc/nginx/sites-available/evolution_api /etc/nginx/sites-enabled/evolution_api
